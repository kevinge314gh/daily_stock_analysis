...
- [修复] 修复本地 CLI 后端（`claude_cli`/`claude_code_cli`）下问股(chat)回复被强制套用报告 JSON schema、导致前端显示原始 JSON 字符串的问题；chat 模式不再注入 `--json-schema`，改为输出自然语言/Markdown，日报生成路径仍保留 schema。
- [改进] 本地 CLI 后端下问股识别到个股时，自动预取实时行情、技术面（趋势/均线/MACD/RSI/量能/支撑压力）与最新新闻并注入 prompt，使 CLI（不支持工具调用）问股基于真实实时数据分析，而非模型训练知识库旧数据；个股识别支持消息中的代码（`600519`/`hk00700`）与股票名称（如「深度分析贵州茅台」经名称→代码解析）；预取为 best-effort，单一数据源失败自动跳过，全部失败时回退为知识库分析。
- [改进] 问股(Web `/chat`)页默认选中策略调整为「长期价值」（仅影响本页初始选择，不改变全局默认策略与日报分析）。
- [chore] 调整 `docker/Dockerfile` 层顺序：`COPY static/` 移至 `ARG CACHEBUST` 之后，使前端产物每次部署必更新进镜像，且前端改动不再触发 Python 依赖层重装，加快重建。
